/*
 * @Author: Your name
 * @Date:   2020-05-27 08:45:09
 * @Last Modified by:   Your name
 * @Last Modified time: 2020-05-28 23:41:19
 */
...
export function cardCreate(props) {
    ...
    props.cardTable.setAllCheckboxAble(CARD.table_code_linkperson, false);
    clearAll(props);
    setByStatus(props);
...
function clearAll(props) {
    ...
    props.cardTable.setTableData(CARD.table_code_linkperson, { rows: [] });
}
...
export function copyClear(props) {
    if (props.getUrlParam('isCopy')) {
        ...
        props.cardTable.setColValue(CARD.table_code_linkperson, PRIMARY_KEY.head_id, { value: null, display: null });
        props.cardTable.setColValue(CARD.table_code_linkperson, PRIMARY_KEY.body_id, { value: null, display: null });

        var num = props.cardTable.getNumberOfRows(CARD.table_code_linkperson, false);
        ...
        props.cardTable.setRowStatusByIndexs(CARD.table_code_linkperson, updateArray);
...
export function cardSave(props) {
    ...
    return new Promise((resolve, rejected) => {
        let flag = props.form.isCheckNow(CARD.form_id) && props.cardTable.checkTableRequired(CARD.table_code_linkperson);
        if (flag) {
            debugger
            //let data = props.createMasterChildData(CARD.page_code, CARD.form_id, [CARD.table_code_linkperson, CARD.table_code_project]);

            let data = props.createExtCardData(CARD.page_code, CARD.form_id, [CARD.table_code_linkperson, CARD.table_code_project]);
            props.validateToSave(data, () => {
                ...
                ajax({
                    ...
                    success: (res) => {
                        ...
                        if (success) {
                            toast({ color: STATUS.success, content: props.json['91H10540-000004'] });/* 国际化处理： 保存成功*/
                            ...
                            if (res.data) {
                                ...
                                if (res.data.body && res.data.body[CARD.table_code_linkperson]) {
                                    props.cardTable.setTableData(CARD.table_code_linkperson, res.data.body[CARD.table_code_linkperson]);
                                }

                                //rongleia 修改
                                if (res.data.body && res.data.body[CARD.table_code_project]) {
                                    props.cardTable.setTableData(CARD.table_code_project, res.data.body[CARD.table_code_project]);
                                }
                            }
            ...
            }, { [CARD.table_code_linkperson]: 'cardTable' });
        }
    });
}
...
export function cardDelete(props) {
    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) {
                toast({ color: STATUS.success, content: props.json['91H10540-000005'] });/* 国际化处理： 删除成功*/
...
export function getCardData(props, id, isRefresh = false, callBack) {
    ...
    if (!isRefresh) {
        ...
        if (cardData) {//有缓存
            ...
            cardData.body && props.cardTable.setTableData(CARD.table_code_linkperson, cardData.body[CARD.table_code_linkperson]);
            setByStatus(props);
            return;
        }
    }
    ajax({
        ...
        success: (res) => {
            ...
            if (success) {
                ...
                if (data && data.bodys) {
                    props.cardTable.setTableData(CARD.table_code_linkperson, data.bodys[CARD.table_code_linkperson]);
                    props.cardTable.setTableData(CARD.table_code_project, data.bodys[CARD.table_code_project]);
                }
                // 更新缓存
                updateCache(PRIMARY_KEY.head_id, id, data, CARD.form_id, DATASOURCE);
...
export function cardCancel(props) {
    ...
    if (id) { //有id切换编辑态
        ...
        props.cardTable.resetTableData(CARD.table_code_linkperson);
    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) { //成功
                ...
                toast({ color: STATUS.warning, content: props.json['91H10540-000015'] });/* 国际化处理： 提交失败*/
            }
        }
    });
}
    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) { //成功
                ...
                toast({ color: STATUS.warning, content: props.json['91H10540-000017'] });/* 国际化处理： 收回失败*/
            }
        }
    });
}
