...
export function buttonClick(props, id, hotkey, isFromSide) {
    switch (id) {
        ...
        case CARD_BUTTON.create:
            props.setUrlParam({ status: 'add', fromPage: 'card', });
			props.form.EmptyAllFormValue('card_body');
			props.form.setFormItemsDisabled('card_body', {
				pk_org_v: false,
				pk_org: false
			})
            props.cardTable.setTableData('card_body', { rows: [] })
            console.log(2);
        ...
        case CARD_BUTTON.delete:
            promptBox({
                ...
                title: this.state.json['91H10545-000000'],/* 国际化处理： 删除*/
                content: this.state.json['91H10545-000001'],/* 国际化处理： 确定删除吗？*/
                beSureBtnClick: () => {
        ...
        case CARD_BUTTON.cancel:
            promptBox({
                color: STATUS.warning,
                title: this.state.json['91H10545-000002'],/* 国际化处理： 取消*/
                content: this.state.json['91H10545-000003'],/* 国际化处理： 是否确认要取消?*/
                beSureBtnClick: () => {
