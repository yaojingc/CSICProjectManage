...
export function loadTreeData() {
    ...
    let treeData = data.map(cur => {
        ...
        return {
            ...
            refname: cur.values.full_seq.value + ' ' + (cur.values.name?cur.values.name.value : ''),
            refpk: cur.values.pk_monthprojectplan_b.value,
            values: {},
...
export function addBrother(e, props, record, index) {
    debugger
    let { plan_level, full_seq, pk_monthprojectplan_h, pk_monthprojectplan_b } = record.values;
    let newpk = new Date().getTime().toString();
    let data = this.treetableData;// props.cardTable.getAllRows(cardTableId);
    index = data.findIndex(cur => cur.rowid == record.rowid);
    let parentindex = 1;//data.findIndex(cur => cur.values.pk_monthprojectplan_b.value == record.pk_monthprojectplan_h.value);
    ...
    data.splice(i1, 0, {
        values: {
            ...
            pk_monthprojectplan_b: { value: newpk },
            pk_monthprojectplan_h: '1q2w3e4r5t',//pk_parent,
        },
        status: '2',
    ...
    if (parentindex != -1) {
        ...
        brothers.splice(brothers.findIndex(cur => cur == pk_monthprojectplan_b.value) + 1, 0, newpk);
...
export function addSub(e, props, record, index) {
    let { plan_level, full_seq, pk_parent, children, pk_monthprojectplan_b } = record.values;
    let newpk = new Date().getTime().toString();
    let data = this.treetableData;// props.cardTable.getAllRows(cardTableId);
    index = data.findIndex(cur => cur.rowid == record.rowid);
    let levelCur = plan_level.value;
    ...
    data.splice(i1, 0, {
        values: {
            ...
            pk_parent: pk_monthprojectplan_b,
            pk_monthprojectplan_b: { value: newpk, display: newpk, }
        },
        status: '2',
        rowid: String(new Date().getTime()).slice(-5) + Math.random().toString(12),
    });
    for (let i = index; i < data.length; i++) {
        ...
        if (i == index) {
            ...
            break;
        } else if (cur.pk_parent.value == pk_monthprojectplan_b.value) {
            cur.line_state = {
                value: ''
            }
        }
...
export function deleteRow(e, props, record, index) {
    ...
    let data = this.treetableData;//props.cardTable.getAllRows(cardTableId);
    index = data.findIndex(cur => cur.rowid == record.rowid);
    let parentindex = data.findIndex(cur => cur.values.pk_monthprojectplan_b.value == pk_parent.value);
    let levelCur = plan_level.value;
    ...
    if (parentindex != -1) {
        let brothers = (data[parentindex].values.children.value || '').split(',').filter(cur => cur != '');
        brothers.splice(brothers.findIndex(cur => cur == pk_monthprojectplan_b.value), 1);
        data[parentindex].values.children.value = brothers.join(',');
    }
...
export function dealDataFromBack(body) {
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        if (pk_parent) {
            temp[pk_parent] = (temp[pk_parent] || []).concat([cur.values.pk_monthprojectplan_b.value]);
        }
    }
    for (let j = 0; j < rows.length; j++) {
        let cur = rows[j];
        if (temp[cur.values.pk_monthprojectplan_b.value]) {
            cur.values.children.value = temp[cur.values.pk_monthprojectplan_b.value].join(',');
        }
    }
