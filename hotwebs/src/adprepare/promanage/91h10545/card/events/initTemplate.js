...
function afterInitTemplet(props) {
	...
	// 此处定义一个变量用来记录此方法走的次数,页面加载的时候只允许走一次
	// let i = PAGE_INDEX_CARD++;
	//console.log(Initcardpageindex());
	//debugger
	//if (Initcardpageindex() < 2) {
		if (status == UISTATE.add) {	
			...
			props.cardTable.addRow(cardTableId, 0, {
				...
				pk_monthprojplan_b: { value: newpk },
				pk_parent: { value: '' }
			},
				true,
				() => {
