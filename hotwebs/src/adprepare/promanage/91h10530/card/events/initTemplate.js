 ...
 * @Last Modified time: 2020-06-22 14:36:16
 */
...
function modifierMeta(props, meta) {
        ...
        meta[CARD.form_id].items.map(item => {
                ...
                if (item.attrcode === 'handingdept' || item.attrcode === 'handingperson') { // 责任人/责任部门 默认参照
                        item.queryCondition = () => {
        ...
        for (let item of Object.keys(meta.gridrelation)) {
                meta[item].items.push({
                        attrcode: 'opr',
                        label: this.state.json['91H10530-000006'],/* 国际化处理： 操作*/
                        itemtype: 'customer',
                        fixed: 'right',
                        className: 'table-opr',
                        visible: true,
