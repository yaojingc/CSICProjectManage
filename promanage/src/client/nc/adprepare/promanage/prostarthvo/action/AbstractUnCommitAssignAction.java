...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggProstartHVO[] commitVOs,
  ...
  protected Object callActionScript(AggProstartHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggProstartHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggProstartHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(ProstartHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggProstartHVO> bills = service.queryBillOfVOByCond(
      ...
      vos = bills.toArray(new AggProstartHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggProstartHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            ProstartHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggProstartHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(ProstartHVOConst.CONST_BILL_STATUS);
