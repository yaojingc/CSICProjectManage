...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggSatisfactionHVO[] commitVOs,
  ...
  protected Object callActionScript(AggSatisfactionHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggSatisfactionHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggSatisfactionHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(SatisfactionHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggSatisfactionHVO> bills = service.queryBillOfVOByCond(
      ...
      vos = bills.toArray(new AggSatisfactionHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggSatisfactionHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            SatisfactionHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggSatisfactionHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(SatisfactionHVOConst.CONST_BILL_STATUS);
