...
import nc.adprepare.promanage.satisfactionhvo.promanage.bean.PageQueryVO;
 ...
 */
public class SatisfactionHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggSatisfactionHVO[] bills =
        (AggSatisfactionHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggSatisfactionHVO[] bills =
        Constructor.construct(AggSatisfactionHVO.class, 0);
   ...
   * @return
   */
  private AggSatisfactionHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggSatisfactionHVO... aggvos) {
    ...
    List<AggSatisfactionHVO> billLst = new ArrayList<AggSatisfactionHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggSatisfactionHVO bill = new AggSatisfactionHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggSatisfactionHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggSatisfactionHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggSatisfactionHVO bill : aggvos) {
