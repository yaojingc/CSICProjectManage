...
import nc.adprepare.promanage.tenprojectplanhvo.promanage.bean.PageQueryVO;
 ...
 */
public class TenprojectplanHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggTenprojectplanHVO[] bills =
        (AggTenprojectplanHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggTenprojectplanHVO[] bills =
        Constructor.construct(AggTenprojectplanHVO.class, 0);
   ...
   * @return
   */
  private AggTenprojectplanHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggTenprojectplanHVO... aggvos) {
    ...
    List<AggTenprojectplanHVO> billLst = new ArrayList<AggTenprojectplanHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggTenprojectplanHVO bill = new AggTenprojectplanHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggTenprojectplanHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggTenprojectplanHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggTenprojectplanHVO bill : aggvos) {
