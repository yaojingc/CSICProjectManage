...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggTenprojectplanHVO[] commitVOs,
  ...
  protected Object callActionScript(AggTenprojectplanHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggTenprojectplanHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggTenprojectplanHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(TenprojectplanHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggTenprojectplanHVO> bills = service.queryBillOfVOByCond(
              AggTenprojectplanHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggTenprojectplanHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggTenprojectplanHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            TenprojectplanHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggTenprojectplanHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(TenprojectplanHVOConst.CONST_BILL_STATUS);
