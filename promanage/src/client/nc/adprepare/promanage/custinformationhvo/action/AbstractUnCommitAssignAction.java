...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggCustinformationHVO[] commitVOs,
  ...
  protected Object callActionScript(AggCustinformationHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggCustinformationHVO[] queryBillsByPks(BillOperatorParam operaParam)
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(CustinformationHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggCustinformationHVO> bills = service.queryBillOfVOByCond(
              AggCustinformationHVO.class, wheresql, true, false);
  ...
  protected Object unCommit(AggCustinformationHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            CustinformationHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggCustinformationHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(CustinformationHVOConst.CONST_BILL_STATUS);
