...
import nc.adprepare.promanage.custinformationhvo.promanage.bean.PageQueryVO;
 ...
 */
public class CustinformationHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggCustinformationHVO[] bills =
        (AggCustinformationHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggCustinformationHVO[] bills =
        Constructor.construct(AggCustinformationHVO.class, 0);
   ...
   * @return
   */
  private AggCustinformationHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggCustinformationHVO... aggvos) {
    ...
    List<AggCustinformationHVO> billLst = new ArrayList<AggCustinformationHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggCustinformationHVO bill = new AggCustinformationHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggCustinformationHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggCustinformationHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggCustinformationHVO bill : aggvos) {
