...
import nc.adprepare.promanage.custinformationhvo.promanage.bean.CommitReturnInfo;
import nc.adprepare.promanage.custinformationhvo.promanage.bean.MultipleResult;
import nc.adprepare.promanage.custinformationhvo.promanage.bean.SingleResult;
...
public abstract class AbstractCommitAssignAction<T extends AbstractBill> implements ICommonAction {
	...
	protected Object buildFontResult(BillOperatorParam operaParam, Object result) {
		// 指派处理
		...
		if (resultVOs.length == 1) {
			BillCardOperator billCardOperator = new BillCardOperator(operaParam.getPageCode());
	...
	protected Object callActionScript(T[] aggVOs, Object content, String billType) throws BusinessException {
		...
		Logger.debug("开始调用动作脚本 ActionName[" + "SAVE" + "] BillType[" + billType + "]...");
		...
		if (result[0] instanceof Map) {
			...
			if (PfUtilBaseTools.PARAM_APPROVEFLOW.equals(type) || PfUtilBaseTools.PARAM_WORKFLOW.equals(type)) {
		...
		if (result[0] instanceof PfProcessBatchRetObject) {
			PfProcessBatchRetObject batchRetObject = (PfProcessBatchRetObject) result[0];
		...
		Logger.debug("调用动作脚本 ActionName[" + "SAVE" + "] BillType[" + billType + "]结束");
		return result;
	}
	 ...
	 */
	protected Object callAutoApproveActionScript(T[] aggVOs, String billType) throws BusinessException {
	...
	private CustomerVO transferData(T aggVO) {
		// 客户聚合VO
		CustomerVO customerVO = new CustomerVO();
		if (aggVO instanceof AggCustinformationHVO) {
			AggCustinformationHVO billVO = (AggCustinformationHVO) aggVO;
			CustinformationHVO headVO = billVO.getParentVO();
			customerVO.setPk_group(headVO.getPk_group());
			customerVO.setPk_org(headVO.getPk_group());
			customerVO.setEnablestate(2);
			customerVO.setCustprop(0);
			customerVO.setDataoriginflag(0);
			customerVO.setIsretailstore(UFBoolean.FALSE);
			customerVO.setFrozenflag(UFBoolean.FALSE);
			customerVO.setPk_country("0001Z010000000079UJJ");// 国家Pk_country Pk_currtype Pk_timezone Pk_format
			customerVO.setPk_custclass(headVO.getDef2());// 客户分类 def2
			customerVO.setName(headVO.getDef3());// 客户名称 def3
			customerVO.setCustcontacts(null);// --0001Z010000000079UJJ 1002Z0100000000001K1 0001Z010000000079U2P
												// FMT0Z000000000000000
			customerVO.setIsvat(UFBoolean.FALSE);
			customerVO.setCode(headVO.getDef1());// 客户编码 def1
			customerVO.setPk_currtype("1002Z0100000000001K1");// TODO
			customerVO.setIssupplier(UFBoolean.FALSE);
			customerVO.setCuststate(1);
			customerVO.setPk_timezone("0001Z010000000079U2P");// TODO
			customerVO.setIsfreecust(UFBoolean.FALSE);
			customerVO.setCusttaxtypes(null);
			customerVO.setPk_format("FMT0Z000000000000000");// TODO
		}
		return customerVO;
	}
	 ...
	 * @throws BusinessException
	 */
	protected Object commit(T vo, Object userobj, String billType) throws BusinessException {
		Object result = null;
		// 判断是否定义审批流
		boolean hasFlow = NCCFlowUtils.hasApproveflowDef(billType, vo);
		// 有，则走提交动作脚本
		if (hasFlow) {
			result = this.callActionScript((T[]) new AbstractBill[] { vo }, userobj, billType);
		}
		// 没有，则提交即审批
		else {
			result = this.callAutoApproveActionScript((T[]) new AbstractBill[] { vo }, billType);
		}
		return result;
	}
	 ...
	 * @throws BusinessException
	 */
	protected MultipleResult doBatchService(T[] aggVOs, Object userobj, String billType) throws BusinessException {
		// 处理总数
		...
		for (T vo : aggVOs) {
			...
			try {
				...
				// 构建成功结果
				singleResult = SingleResult.buildSuccessResult(vo.getPrimaryKey(), null, null, null);
				// 构建成功的结果
				hasSuccFlag = true;
				// 成功条数+1
				successNum++;
				...
				// 构建失败结果
				singleResult = SingleResult.buildErrResult(vo.getPrimaryKey(), null, null, errMess);
			}
			// 汇总处理结果
		...
		// 构建批量操作结果
		MultipleResult multipleResult = MultipleResult.buildResult(hasSuccFlag, hasErrFlag, resultList, errMsgDetail,
				total, errorNum, successNum);
	...
	protected Object doCommit(T[] commitVOs, Object userobj, String billType) throws BusinessException {
		Object result = null;
		...
		// TODO
		if (CSICCommonUtils.isNotEmpty(commitVOs)) {
			// 开始组装数据,将客户信息登记转化为客户档案
			CustomerVO customerVO = this.transferData(commitVOs[0]);
			// 单据标准审批走完后，推单到客户档案中
			CustomerService cs = new CustomerService();// 客户档案后台服务接口
			try {
				customerVO = cs.insertCustomerVO(customerVO, false);
	...
	protected AggCustinformationHVO[] queryBillsByPks(BillOperatorParam operaParam) throws BusinessException {
		AggCustinformationHVO[] vos = null;
		String[] pks = operaParam.getPks();
		if (pks.length != 0) {
			// 根据参数查询结果
			IMDPersistenceQueryService service = CommonUtil.getMDPersistenceQueryService();
			String wheresql = SQLUtil.buildSqlForIn(CustinformationHVOConst.CONST_PARENT_PRIMARYKEY, pks);
			...
			vos = bills.toArray(new AggCustinformationHVO[0]);
		}
		return vos;
	}
