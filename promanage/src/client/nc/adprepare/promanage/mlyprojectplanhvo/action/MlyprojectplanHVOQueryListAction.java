 ...
 */
public class MlyprojectplanHVOQueryListAction implements ICommonAction {
	...
	public Object doAction(IRequest paramIRequest) {
		...
		try {
			...
			AggMlyprojectplanHVO[] aggvos = CommonUtil.getFinancingcostMaintainService().query(scheme);
			// 查询服务(scheme);
			if ((aggvos != null) && (aggvos.length > 0)) {
				// 3、处理分页
				String[] pks = this.getPks(aggvos);
				AggMlyprojectplanHVO[] bills = this.getDefaulePageBill(queryParam, aggvos);
	...
	private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
		AggMlyprojectplanHVO[] bills = (AggMlyprojectplanHVO[]) pagevo.getCurrentPageBills();
	...
	private PageQueryVO createNullPage() {
		...
		AggMlyprojectplanHVO[] bills = Constructor.construct(AggMlyprojectplanHVO.class, 0);
	 ...
	 * @return
	 */
	private AggMlyprojectplanHVO[] getDefaulePageBill(QueryTreeFormatVO info, AggMlyprojectplanHVO... aggvos) {
		...
		List<AggMlyprojectplanHVO> billLst = new ArrayList<AggMlyprojectplanHVO>();
		for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
			AggMlyprojectplanHVO bill = new AggMlyprojectplanHVO();

			bill.setParent(aggvos[i].getParent());
			billLst.add(bill);
		}
		return billLst.toArray(new AggMlyprojectplanHVO[0]);
	}
	 ...
	 * @return
	 */
	private String[] getPks(AggMlyprojectplanHVO... aggvos) {
		List<String> pks = new ArrayList<String>();
		for (AggMlyprojectplanHVO bill : aggvos) {
