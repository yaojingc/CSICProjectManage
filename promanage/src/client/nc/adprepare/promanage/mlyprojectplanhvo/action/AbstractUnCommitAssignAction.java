...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggMlyprojectplanHVO[] commitVOs,
  ...
  protected Object callActionScript(AggMlyprojectplanHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggMlyprojectplanHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggMlyprojectplanHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(MlyprojectplanHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggMlyprojectplanHVO> bills = service.queryBillOfVOByCond(
              AggMlyprojectplanHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggMlyprojectplanHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggMlyprojectplanHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            MlyprojectplanHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggMlyprojectplanHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(MlyprojectplanHVOConst.CONST_BILL_STATUS);
