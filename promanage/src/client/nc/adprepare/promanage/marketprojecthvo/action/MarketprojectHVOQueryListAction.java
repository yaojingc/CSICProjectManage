...
import nc.adprepare.promanage.marketprojecthvo.promanage.bean.PageQueryVO;
 ...
 */
public class MarketprojectHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggMarketprojectHVO[] bills =
        (AggMarketprojectHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggMarketprojectHVO[] bills =
        Constructor.construct(AggMarketprojectHVO.class, 0);
   ...
   * @return
   */
  private AggMarketprojectHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggMarketprojectHVO... aggvos) {
    ...
    List<AggMarketprojectHVO> billLst = new ArrayList<AggMarketprojectHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggMarketprojectHVO bill = new AggMarketprojectHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggMarketprojectHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggMarketprojectHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggMarketprojectHVO bill : aggvos) {
