 ...
 */
public class CustregisterHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggCustregisterHVO[] bills =
        (AggCustregisterHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggCustregisterHVO[] bills =
        Constructor.construct(AggCustregisterHVO.class, 0);
   ...
   * @return
   */
  private AggCustregisterHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggCustregisterHVO... aggvos) {
    ...
    List<AggCustregisterHVO> billLst = new ArrayList<AggCustregisterHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggCustregisterHVO bill = new AggCustregisterHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggCustregisterHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggCustregisterHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggCustregisterHVO bill : aggvos) {
