 ...
 */
public class CustregisterHVOQueryByPksAction implements ICommonAction {
  ...
  protected Object transGrid(PageQueryInfo info,
    ...
    int i = 0;
    for (AggCustregisterHVO bill : bills) {
   ...
   */
  private Collection<AggCustregisterHVO> queryBills(PageQueryInfo queryParam)
      throws MetaDataException {
    // 1、根据参数查询结果
    IMDPersistenceQueryService service = CommonUtil.getMDPersistenceQueryService();
    String wheresql =
        SQLUtil.buildSqlForIn("pk_custregister_h",queryParam.getPks());
    Collection<AggCustregisterHVO> bills =
        service.queryBillOfVOByCond(AggCustregisterHVO.class, wheresql, false,
            false);
    return bills;
  }
}
