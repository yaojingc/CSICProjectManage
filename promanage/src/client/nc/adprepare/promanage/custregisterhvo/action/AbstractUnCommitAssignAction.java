...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggCustregisterHVO[] commitVOs,
  ...
  protected Object callActionScript(AggCustregisterHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggCustregisterHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggCustregisterHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(CustregisterHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggCustregisterHVO> bills = service.queryBillOfVOByCond(
              AggCustregisterHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggCustregisterHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggCustregisterHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            CustregisterHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggCustregisterHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(CustregisterHVOConst.CONST_BILL_STATUS);
