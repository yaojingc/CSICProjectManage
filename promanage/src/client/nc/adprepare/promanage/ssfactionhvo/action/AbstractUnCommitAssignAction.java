...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggSsfactionHVO[] commitVOs,
  ...
  protected Object callActionScript(AggSsfactionHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggSsfactionHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggSsfactionHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(SsfactionHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggSsfactionHVO> bills = service.queryBillOfVOByCond(
      ...
      vos = bills.toArray(new AggSsfactionHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggSsfactionHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            SsfactionHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggSsfactionHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(SsfactionHVOConst.CONST_BILL_STATUS);
