...
import nc.adprepare.promanage.ssfactionhvo.promanage.bean.PageQueryVO;
 ...
 */
public class SsfactionHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggSsfactionHVO[] bills =
        (AggSsfactionHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggSsfactionHVO[] bills =
        Constructor.construct(AggSsfactionHVO.class, 0);
   ...
   * @return
   */
  private AggSsfactionHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggSsfactionHVO... aggvos) {
    ...
    List<AggSsfactionHVO> billLst = new ArrayList<AggSsfactionHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggSsfactionHVO bill = new AggSsfactionHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggSsfactionHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggSsfactionHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggSsfactionHVO bill : aggvos) {
