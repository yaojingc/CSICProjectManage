...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggXunprojectplanHVO[] commitVOs,
  ...
  protected Object callActionScript(AggXunprojectplanHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggXunprojectplanHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggXunprojectplanHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(XunprojectplanHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggXunprojectplanHVO> bills = service.queryBillOfVOByCond(
              AggXunprojectplanHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggXunprojectplanHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggXunprojectplanHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            XunprojectplanHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggXunprojectplanHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(XunprojectplanHVOConst.CONST_BILL_STATUS);
    ...
    if (hasFlow) {
      result = this.callActionScript(commitVOs,
              XunprojectplanHVOConst.CONST_BILLTYPE_COST);
    }
    // 没有，则收回即弃审
    else {
