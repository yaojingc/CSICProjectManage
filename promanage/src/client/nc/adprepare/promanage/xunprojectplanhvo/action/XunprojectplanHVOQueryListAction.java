...
import nc.adprepare.promanage.xunprojectplanhvo.promanage.bean.PageQueryVO;
 ...
 */
public class XunprojectplanHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggXunprojectplanHVO[] bills =
        (AggXunprojectplanHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggXunprojectplanHVO[] bills =
        Constructor.construct(AggXunprojectplanHVO.class, 0);
   ...
   * @return
   */
  private AggXunprojectplanHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggXunprojectplanHVO... aggvos) {
    ...
    List<AggXunprojectplanHVO> billLst = new ArrayList<AggXunprojectplanHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggXunprojectplanHVO bill = new AggXunprojectplanHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggXunprojectplanHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggXunprojectplanHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggXunprojectplanHVO bill : aggvos) {
