...
import nc.adprepare.promanage.profinishhvo.promanage.bean.PageQueryVO;
 ...
 */
public class ProfinishHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggProfinishHVO[] bills =
        (AggProfinishHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggProfinishHVO[] bills =
        Constructor.construct(AggProfinishHVO.class, 0);
   ...
   * @return
   */
  private AggProfinishHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggProfinishHVO... aggvos) {
    ...
    List<AggProfinishHVO> billLst = new ArrayList<AggProfinishHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggProfinishHVO bill = new AggProfinishHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggProfinishHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggProfinishHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggProfinishHVO bill : aggvos) {
