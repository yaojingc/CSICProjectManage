...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggTendaysprojplanHVO[] commitVOs,
  ...
  protected Object callActionScript(AggTendaysprojplanHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggTendaysprojplanHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggTendaysprojplanHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(TendaysprojplanHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggTendaysprojplanHVO> bills = service.queryBillOfVOByCond(
              AggTendaysprojplanHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggTendaysprojplanHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggTendaysprojplanHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            TendaysprojplanHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggTendaysprojplanHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(TendaysprojplanHVOConst.CONST_BILL_STATUS);
    ...
    if (hasFlow) {
      result = this.callActionScript(commitVOs,
              TendaysprojplanHVOConst.CONST_BILLTYPE_COST);
    }
    // 没有，则收回即弃审
    else {
