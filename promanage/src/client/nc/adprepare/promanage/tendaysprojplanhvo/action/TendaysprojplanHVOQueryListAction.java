...
import nc.adprepare.promanage.tendaysprojplanhvo.promanage.bean.PageQueryVO;
 ...
 */
public class TendaysprojplanHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggTendaysprojplanHVO[] bills =
        (AggTendaysprojplanHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggTendaysprojplanHVO[] bills =
        Constructor.construct(AggTendaysprojplanHVO.class, 0);
   ...
   * @return
   */
  private AggTendaysprojplanHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggTendaysprojplanHVO... aggvos) {
    ...
    List<AggTendaysprojplanHVO> billLst = new ArrayList<AggTendaysprojplanHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggTendaysprojplanHVO bill = new AggTendaysprojplanHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggTendaysprojplanHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggTendaysprojplanHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggTendaysprojplanHVO bill : aggvos) {
