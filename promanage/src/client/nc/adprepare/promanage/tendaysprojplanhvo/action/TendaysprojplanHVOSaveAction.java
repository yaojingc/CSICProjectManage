 ...
 */
public class TendaysprojplanHVOSaveAction implements ICommonAction {
...
AggTendaysprojplanHVO vo = billCardOperator.toBill(paramIRequest);
      this.doBefore(vo);
      // 2、调用单据的保存动作脚本（savebase），得到保存后结果
AggTendaysprojplanHVO rtnObj = this.callActionScript(vo);
  ...
  protected String getActionCode() {
    return TendaysprojplanHVOConst.CONST_ACTION_SAVEBASE;
  }
   ...
   */
  private AggTendaysprojplanHVO callActionScript(AggTendaysprojplanHVO... aggVOs)
      throws BusinessException {
    ...
    String wheresql =
        "pk_tendaysprojplan_h" + "='"
            + ((AggTendaysprojplanHVO) result[0]).getPrimaryKey() + "'";
    ...
    return bills.toArray(new AggTendaysprojplanHVO[0])[0];
  }
   ...
   * @param vo
   */
  private void doBefore(AggTendaysprojplanHVO vo) {
    ...
    if ((parentPk != null) && !"".equals(parentPk)) {
      ...
      if (allchildren != null) {
        for (CircularlyAccessibleValueObject obj : allchildren) {
          obj.setAttributeValue("pk_tendaysprojplan_h", parentPk);
        }
      }
    }
  }
