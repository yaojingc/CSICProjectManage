...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggMonprojectplanHVO[] commitVOs,
  ...
  protected Object callActionScript(AggMonprojectplanHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggMonprojectplanHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggMonprojectplanHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(MonprojectplanHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggMonprojectplanHVO> bills = service.queryBillOfVOByCond(
              AggMonprojectplanHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggMonprojectplanHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggMonprojectplanHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            MonprojectplanHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggMonprojectplanHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(MonprojectplanHVOConst.CONST_BILL_STATUS);
