...
import nc.adprepare.promanage.monprojectplanhvo.promanage.bean.PageQueryVO;
 ...
 */
public class MonprojectplanHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggMonprojectplanHVO[] bills =
        (AggMonprojectplanHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggMonprojectplanHVO[] bills =
        Constructor.construct(AggMonprojectplanHVO.class, 0);
   ...
   * @return
   */
  private AggMonprojectplanHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggMonprojectplanHVO... aggvos) {
    ...
    List<AggMonprojectplanHVO> billLst = new ArrayList<AggMonprojectplanHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggMonprojectplanHVO bill = new AggMonprojectplanHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggMonprojectplanHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggMonprojectplanHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggMonprojectplanHVO bill : aggvos) {
